	...
	public Object getSingleton(String beanName, ObjectFactory<?> singletonFactory) {
		...
		synchronized (this.singletonObjects) {
			...
			if (singletonObject == null) {
				...
				// 把当前正在创建的 beanName 添加到 singletonsCurrentlyInCreation 中
				// singletonsCurrentlyInCreation 是一个当前正在创建的 Bean 的 beanName 集合
				...
				try {
					// singletonFactory 就是外面传入的 lambda 表达式， 执行 lambda 表达式
					// 创建 bean
					singletonObject = singletonFactory.getObject();
					newSingleton = true;
				}
					...
					// 讲刚才正在创建的 beanName 从 singletonsCurrentLyInCreation 中移除

...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	static {
		...
		ContextClosedEvent.class.getName(); //主动记载 ContextClosedEvent 避免奇怪的类加载问题
	}
	...
	public void refresh() throws BeansException, IllegalStateException {
		...
		synchronized (this.startupShutdownMonitor) {
			...
			try {
				...
				//     1）、子类通过重写这个方法来在 BeanFactory 创建并预准备完成以后作进一步的设置
				...
				//    BeanFactory 的后置处理器， 在 BeanFactory 标准初始化之后执行的
				//    两个接口：BeanFactoryPostProcessor、BeanDefinitionRegistryPostProcessor
				invokeBeanFactoryPostProcessors(beanFactory);

				// Register bean processors that intercept bean creation.
				// 6. 注册 BeanPostProcessors 的后置处理器 [intercept bean creation.]
	...
	protected void initMessageSource() {
		...
		// 2. 判断是否有 ID 为 messageSource 的组件
	...
	protected void finishBeanFactoryInitialization(ConfigurableListableBeanFactory beanFactory) {
		...
		// 如果 BeanFactory 中存在 ConversionService ， 则设置 BeanFactory 的 ConversionService
		// ConversionService 是用来做类型转换的
		...
		// 冻结所有的已经注册了的 BeanDefinition, 后续不能修改和处理
